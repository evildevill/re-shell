---
name: general-re
description: Core reverse-engineering triage, artifact handling, native binary analysis, firmware, hardware, USB, FPGA, and shared tool workflows.
---
...
This environment is organized into a **general-purpose core** (this skill) and **discipline-specific skills** that auto-activate based on context. Skills provide specialized tool documentation, workflows, and notes for their domain.

### Available Disciplines

| Skill | Path | Activates On |
|-------|------|-------------|
| Android RE | `skills/android-re/SKILL.md` | APK, DEX, smali, ADB, Android app analysis |
| Windows RE | `skills/windows-re/SKILL.md` | PE, .exe, .dll, .sys, .NET, Windows binary analysis |
| Web RE | `skills/web-re/SKILL.md` | Protobuf, gRPC, HAR, HTTP API, WebSocket, TLS fingerprint, web scraping |
| Binary Triage | `skills/binary-triage/SKILL.md` | Unknown files, archives, disk images, suspicious samples |
| Linux/ELF RE | `skills/linux-elf-re/SKILL.md` | ELF binaries, shared libraries, loaders, native Unix code |
| Firmware RE | `skills/firmware-re/SKILL.md` | Flash dumps, bootloaders, filesystems, firmware updates |
| Network/Protocol RE | `skills/network-protocol-re/SKILL.md` | PCAP, unknown protocols, framing, state machines |
| Malware Analysis | `skills/malware-analysis/SKILL.md` | Defensive malware triage, memory images, IOCs |
| Ghidra Workflows | `skills/ghidra-workflows/SKILL.md` | Decompilation, annotation, PyGhidra, headless analysis |
| Frida Workflows | `skills/frida-workflows/SKILL.md` | Authorized runtime instrumentation and hooks |

### Adding a New Discipline

1. Create `skills/<discipline>/SKILL.md` with standard front matter (`name` and a trigger-rich `description`).
